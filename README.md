...
✅ Modelo 3D real (carro de carreras, .glb) integrado, escalado y con animación de giro (efecto vitrina/turntable)
✅ Marcador AR (`targets/morral.mind`) compilado desde la foto real del morral
✅ Fondo de cámara real funcionando (bug resuelto: la librería MindAR v1.2.5 tiene un problema conocido cuando se combinan las opciones `uiScanning: no` / `uiLoading: no` — quitarlas resuelve el fondo negro)
🎉 **Proyecto completo y funcional — listo para usar**

### Nota técnica sobre el bug del fondo negro
Si en algún momento vuelves a ver fondo negro en vez de la cámara real, revisa
que la línea `mindar-image="..."` dentro de `index.html` **no** incluya
`uiScanning: no; uiLoading: no;` — esa combinación bloquea el video de
fondo en esta versión de la librería. La versión actual del archivo ya
tiene esto corregido.
